...
`sw.js`, the files it precaches and the font licence, never the tests, tools or docs. The workflow's actions are
pinned to commit SHAs. Once a week Dependabot (`.github/dependabot.yml`) opens a pull request when any of them, or
`@playwright/test`, has a new release; merging it is what deploys. After a Playwright bump, rerun the one-off
`npx playwright install --only-shell chromium` above for its browser build.
